...
class QuoteRequestAdmin(admin.ModelAdmin):
    list_display = ('id', 'user', 'insurance_type', 'sum_insured', 'city', 'created_at', 'view_comparison')
    ...
    def get_urls(self):
        urls = super().get_urls()
        custom_urls = [
            path('<path:object_id>/comparison/', self.admin_site.admin_view(self.comparison_view), name='comparison-matrix'),
        ]
        return custom_urls + urls

    def comparison_view(self, request, object_id):
        obj = self.get_object(request, object_id)
        context = dict(
            self.admin_site.each_context(request),
            title=f"Comparison Matrix: {obj}",
            object=obj,
            quotes=obj.quotes.all().order_by('-comparison_score'),
        )
        return TemplateResponse(request, "admin/api_set1/quoterequest/comparison_matrix.html", context)

    def view_comparison(self, obj):
        return format_html(
            '<a class="button" href="{}">Comparison</a>',
            f"{obj.id}/comparison/"
        )
    view_comparison.short_description = "Analysis"
